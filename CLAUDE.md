...
src/
  ...
  lib/home.js                       # Hero slideshow, themes carousel, atlas grid, featured frame (imports critical.css)
...
css/critical.css                    # Home-page-only CSS (base, header, hero, atlas, themes, featured, footer)
...
- **Per-page CSS**: `home.js` imports `css/critical.css` (~42 KB / ~8 KB gzip), `gallery.js` imports `css/style.css` (~53 KB / ~9.9 KB gzip). Vite emits one CSS file per chunk so each page only loads its own
