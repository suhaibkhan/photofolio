...
- **Split lib modules**: `src/lib/shared.js` (mobile menu, header scroll, scroll reveal, location helpers, paths re-exports, data), `src/lib/home.js` (hero, themes, atlas), `src/lib/gallery.js` (gallery filters, lightbox). Page entries import only the modules they need so each page ships only its own JS
- **Per-page CSS**: `home.js` imports `css/critical.css` (home + footer + responsive + utilities); `gallery.js` imports `css/style.css` (full sheet). Vite emits one CSS chunk per JS chunk, so each page only loads what it needs
- **Static pre-rendering**: a custom Vite plugin (`vite-plugins/prerender-index.js`) reads `data/photos-local.json` at build/dev time and inlines the atlas tiles, theme plates, hero first slide, and hero WebP preload links into `index.html`. The home page paints real content before `app.js` runs
...
vite-plugins/prerender-index.js     # Reads photos-local.json, injects atlas/themes/hero markup into index.html
src/
  ...
  lib/shared.js                     # Mobile menu, header scroll, scroll reveal, helpers, data import
  lib/home.js                       # Hero slideshow, themes carousel, atlas grid (imports critical.css)
  lib/gallery.js                    # Gallery filters, masonry grid, lightbox (imports style.css)
...
css/critical.css                    # Home-page-only CSS (base, header, hero, atlas, themes, footer)
css/style.css                       # Full sheet — gallery + lightbox + everything in critical.css
...
public/images/hero/                 # 2400px WebP variants — used by hero slideshow (only for hero/mobileHero photos)
...
- WebP covers (1200px max edge) live at `public/images/covers/<name>.webp` — used by atlas, themes, and gallery tiles. `coverWebp()` in `src/lib/paths.js` derives the cover path from the original `src` by swapping `/photos` → `/covers` and `.jpg` → `.webp`
- WebP hero variants (2400px max edge, q=85) live at `public/images/hero/<name>.webp` — used by the hero slideshow only. `heroWebp()` in `src/lib/paths.js` follows the same pattern but swaps to `/hero`. Generated only for photos with `hero === true` or `mobileHero === true`
...
- **Image pipeline** (`scripts/compress-covers.mjs`, run via `npm run compress:covers`):
  - Pass 1 — **covers** at 1200px / q=80 → `public/images/covers/<name>.webp` for every photo + every location/category cover. Used by atlas, themes, gallery masonry
  - Pass 2 — **hero** at 2400px / q=85 → `public/images/hero/<name>.webp` only for photos with `hero` or `mobileHero` true. Used by the hero slideshow
...
- **Static pre-rendering**: `vite-plugins/prerender-index.js` runs in both `serve` (dev) and `build`. For `index.html` only (gallery is left alone), it replaces five `<!--PRERENDER:*-->` markers with HTML for the atlas grid, themes track, hero first slide, hero title/location text, and `<link rel="preload">` tags for the hero WebP (mobile + desktop variants with appropriate `media` query). In dev mode, changes to `data/photos-local.json` trigger a full reload
...
- **Per-page CSS**: `home.js` imports `css/critical.css` (~35 KB / ~6.7 KB gzip), `gallery.js` imports `css/style.css` (~53 KB / ~9.9 KB gzip). Vite emits one CSS file per chunk so each page only loads its own
